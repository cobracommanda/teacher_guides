...
function replaceLastHtmlElement(htmlString, newTag, append) {
  const regex = /(<[^>]+>[^<]*<\/[^>]+>)\s*$/;
  const match = htmlString.match(regex);

  if (match) {
    if (append) {
      return `${htmlString.replace(regex, newTag)} ${match[0]}`;
    } else {
...
// const result = replaceLastHtmlElement(htmlString, newTag, true);
// const result2 = replaceLastHtmlElement(htmlString, newTag, false);
// console.log(result);
// console.log(result2);
